  ...
  try {
    ...
    if (interaction.customId.startsWith("spin_")) {
      ...
      try {
        ...
        // Re-fetch the message to get fresh, up-to-date reaction data
        // (targetMessage may have stale/empty cache if bot just started)
        const freshMessage = await reactionChannel.messages.fetch(
          REQUIRED_REACTION_MESSAGE_ID
        );
